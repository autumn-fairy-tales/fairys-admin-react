...
export const FairysFullScreen = (props: FairysFullScreenProps) => {
  ...
  const className = useMemo(() => {
    return clsx(
      ...
      {
        open: open,
        'fairys:absolute fairys:top-0 fairys:left-0 fairys:right-0 fairys:bottom-0': !!open,
      },
    );
  }, [open, cls]);
  ...
  const closeClassName = useMemo(() => {
    return clsx(
      'fairys_admin_full_screen_close fairys:transition-all fairys:duration-300 fairys:justify-center fairys:items-center fairys:flex fairys:absolute fairys:top-0 fairys:right-0 fairys:w-[38px] fairys:h-[38px] fairys:cursor-pointer',
      {},
    );
  }, [open]);
    ...
    <div {...rest} className={className}>
      {open ? (
        <div title="退出全屏" className={closeClassName} onClick={onClose}>
          <span className="fairys:absolute fairys:top-[4px] fairys:right-[4px] fairys:size-[20px] fairys:icon-[mdi-light--fullscreen-exit]" />
        </div>
      ) : (
        <Fragment />
      )}
